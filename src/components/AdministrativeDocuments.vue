          ...
          <v-row>
            <v-col cols="12">
              <v-select
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.administrativeDocumentTypeId"
              ></v-select>
            </v-col>
          </v-row>
          <v-row>
...
<script setup>
  ...
  function loadData() {
    Promise.all([
      axios.get('/api/administrative-documents/')
    ])
    .then((responses) => {
  ...
  function createNewDocument () {
    return {
      id: 0,
      administrativeDocumentTypeId: null,
      forFamiliarization: {
        id: null,
        name: '',
      },
  ...
  function addDocument() {
    Promise.all([
      ...
      axios.get('/api/administrative-document-types/')
    ])
    .then((responses) => {
  ...
  function removeDocument(item) {
    Promise.all([axios.post(`/api/administrative-documents/delete`, item)])
    .then((responses) => {
  ...
  function editDocument(item) {
    ...
    .then((responses) => {
      ...
      documentModel.value = cloneDeep({
        ...
        forFamiliarization: tempDocumentModel.value.forFamiliarization,
        forExecution: tempDocumentModel.value.forExecution,
        supervisor: tempDocumentModel.value.supervisor,
        ...
        administrativeDocumentTypeId: tempDocumentModel.value.administrativeDocumentTypeId,
  ...
  async function saveDocument() {
    if (isEditingDocument.value) {
      ...
      Promise.all([axios.post("/api/administrative-documents/update", data)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/administrative-documents/create", documentModel.value)])
      .then((responses) => { 
  ...
  async function saveUser(user) {
    ...
    } else if (state.value == 'execution') {
      documentModel.value.forExecution = user;
      documentModel.value.forExecutionId = user.id;
    } else if (state.value == 'familiarization') {
      documentModel.value.forFamiliarization = user;
      documentModel.value.forFamiliarizationId = user.id;
    } else if (state.value == 'supervisor') {
      documentModel.value.supervisor = user;
