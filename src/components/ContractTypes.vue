<template>
  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card :title="`${isEditing ? 'Изменение' : 'Добавление'} типа договора`">
...
<script setup>
  ...
  function removeItem(id) {
    Promise.all([axios.post(`/api/contract-types/delete/${id}`)])
    .then((responses) => {
  ...
  async function save() {
    ...
    if (isEditing.value) {
      ...
      Promise.all([axios.post("/api/contract-types/update", formModel.value)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/contract-types/create", formModel.value)])
      .then((responses) => { 
        var serverContractType = responses[0].data;
        console.log(serverContractType);     
        items.value.push(serverContractType);
      });
    }
