          ...
          <v-row>
            <v-col cols="12">
              <v-select
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.addresserId"
              ></v-select>
            </v-col>
          </v-row>
          <v-row>
...
<script setup>
  ...
  function loadData() {
    Promise.all([
      axios.get('/api/incoming-correspondences/')
    ])
    .then((responses) => {
  ...
  function removeDocument(item) {
    Promise.all([axios.post(`/api/incoming-correspondences/delete`, item)])
    .then((responses) => {
  ...
  async function saveDocument() {
    if (isEditingDocument.value) {
      ...
      Promise.all([axios.post("/api/incoming-correspondences/update", data)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/incoming-correspondences/create", documentModel.value)])
      .then((responses) => { 
