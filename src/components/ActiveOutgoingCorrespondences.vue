      ...
      <v-row>
        <v-col cols="12">
          <v-autocomplete
            ...
            item-title="name"
            item-value="id"
            v-model="documentModel.preparedBy"
            readonly
          >
          </v-autocomplete>
        </v-col>
...
<script setup>
  ...
  function loadData() {
    Promise.all([
      axios.get('/api/active/outgoing-correspondences/')
    ])
    .then((responses) => {
  ...
  async function acceptDocument() {
    Promise.all([axios.post("/api/active/outgoing-correspondences/accept", documentModel.value)])
    .then((responses) => {           
