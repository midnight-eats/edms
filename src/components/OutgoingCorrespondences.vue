      ...
      <v-tabs-window-item value="document">
        <container>
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field 
                v-model="documentModel.addressee_name"
                label="ФИО адресата"
                required
              ></v-text-field>
            </v-col>
          ...
          <v-row>
            <v-col cols="12">
              <v-select
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.deliveryMethodId"
              ></v-select>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12">
              <v-autocomplete
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.preparedById"
                readonly
                @click="addUser('preparedby')"
              >
              </v-autocomplete>
            </v-col>
          </v-row>
...
<script setup>
  ...
  function loadData() {
    Promise.all([
      axios.get('/api/outgoing-correspondences/')
    ])
    .then((responses) => {
  ...
  function addDocument() {
    ...
    .then((responses) => {
      ...
      deliveryMethods.value = responses[2].data;
  ...
  function removeDocument(item) {
    Promise.all([axios.post(`/api/outgoing-correspondences/delete`, item)])
    .then((responses) => {
  ...
  async function saveDocument() {
    if (isEditingDocument.value) {
      ...
      Promise.all([axios.post("/api/outgoing-correspondences/update", data)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/outgoing-correspondences/create", documentModel.value)])
      .then((responses) => { 
