<template>
  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card :title="`${isEditing ? 'Изменение' : 'Добавление'} ${dialogTitle}`">
...
<script setup>
  ...
  function removeItem(id) {
    Promise.all([axios.post(`${props.apiEndpoint}/delete/${id}`)])
    .then((responses) => {
  ...
  async function save() {
    ...
    if (isEditing.value) {
      ...
      Promise.all([axios.post(`${props.apiEndpoint}/update`, formModel.value)])
      .then((responses) => {           
      ...
      Promise.all([axios.post(`${props.apiEndpoint}/create`, formModel.value)])
      .then((responses) => { 
        var serverItem = responses[0].data;
