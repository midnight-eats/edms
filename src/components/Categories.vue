...
<script setup>
  ...
  function removeItem(id) {
    Promise.all([axios.post(`/api/categories/delete/${id}`)])
    .then((responses) => {
  ...
  function save() {
    if (isEditing.value) {
      Promise.all([axios.post("/api/categories/update", formModel.value)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/categories/create", formModel.value)])
      .then((responses) => { 
        var serverCategory = responses[0].data;
        console.log(serverCategory);     
        items.value.push(serverCategory);
      });
    }
