<template>
  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card :title="`${isEditing ? 'Изменение' : 'Добавление'} типа служебной записки`">
...
<script setup>
  ...
  function removeItem(id) {
    Promise.all([axios.post(`/api/memo-types/delete/${id}`)])
    .then((responses) => {
  ...
  async function save() {
    ...
    if (isEditing.value) {
      ...
      Promise.all([axios.post("/api/memo-types/update", formModel.value)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/memo-types/create", formModel.value)])
      .then((responses) => { 
        var serverMemoType = responses[0].data;
        console.log(serverMemoType);     
        items.value.push(serverMemoType);
      });
    }
