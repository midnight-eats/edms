          ...
          <v-row>
            <v-col cols="12">
              <v-select
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.internalDocumentTypeId"
              ></v-select>
            </v-col>
          </v-row>
          <v-row>
          ...
          <v-row>
            <v-col cols="12">
              <v-autocomplete
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.forExecutionId"
                readonly
                @click="addUser('execution')"
              >
              </v-autocomplete>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12">
              <v-autocomplete
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.forFamiliarizationId"
                readonly
                @click="addUser('familiarization')"
              >
              </v-autocomplete>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12">
              <v-autocomplete
                ...
                item-title="name"
                item-value="id"
                v-model="documentModel.supervisorId"
                readonly
...
<script setup>
  ...
  function loadData() {
    Promise.all([
      axios.get('/api/internal-documents/')
    ])
    .then((responses) => {
  ...
  function addDepartment(currState) {  
    departmentState.value = currState;

    departmentDialog.value = true;
  }

  function saveDepartment(item) {  
    if (departmentState.value === 'addressee')
      documentModel.value.addressee = item;
    else if (departmentState.value === 'addresser')
      documentModel.value.addresser = item;
  ...
  function createNewDocument () {
    return {
      ...
      internalDocumentTypeId: null,
      forFamiliarization: {
        id: null,
        name: '',
      },
      forFamiliarizationId: null, 
      forExecution: {
        id: null,
        name: '',
      },
  ...
  function addDocument() {
    Promise.all([
      ...
      axios.get('/api/internal-document-types/')
    ])
    .then((responses) => {
  ...
  function removeDocument(item) {
    Promise.all([axios.post(`/api/internal-documents/delete`, item)])
    .then((responses) => {
  ...
  function editDocument(item) {
    ...
    .then((responses) => {
      ...
      documentModel.value = cloneDeep({
        ...
        addressee: tempDocumentModel.value.addressee,
        addresser: tempDocumentModel.value.addresser,
        forFamiliarizationId: tempDocumentModel.value.forFamiliarizationId,
        forExecutionId: tempDocumentModel.value.forExecutionId,
        supervisorId: tempDocumentModel.value.supervisorId,
        internalDocumentTypeId: tempDocumentModel.value.internalDocumentTypeId,
  ...
  async function saveDocument() {
    if (isEditingDocument.value) {
      ...
      Promise.all([axios.post("/api/internal-documents/update", data)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/internal-documents/create", documentModel.value)])
      .then((responses) => { 
  ...
  async function saveUser(user) {
    if (userState.value === 'routestageuser') {    
    ...
    } else if (userState.value === 'author') {
      documentModel.value.document.author = user;
      documentModel.value.document.authorId = user.id;
    } else if (userState.value === 'execution')
      documentModel.value.forExecutionId = user.id;
    else if (userState.value === 'familiarization')
      documentModel.value.forFamiliarizationId = user.id;
    else if (userState.value === 'supervisor')
