<template>
  ...
  <v-dialog v-model="dialog" max-width="500">
    <v-card :title="`${isEditing ? 'Изменение' : 'Добавление'} типа ОРД`">
...
<script setup>
  ...
  function removeItem(id) {
    Promise.all([axios.post(`/api/administrative-document-types/delete/${id}`)])
    .then((responses) => {
  ...
  async function save() {
    ...
    if (isEditing.value) {
      ...
      Promise.all([axios.post("/api/administrative-document-types/update", formModel.value)])
      .then((responses) => {           
      ...
      Promise.all([axios.post("/api/administrative-document-types/create", formModel.value)])
      .then((responses) => { 
        var serverAdministrativeDocumentType = responses[0].data;
        console.log(serverAdministrativeDocumentType);     
        items.value.push(serverAdministrativeDocumentType);
      });
    }
